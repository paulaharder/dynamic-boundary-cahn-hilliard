function main_cahn_hilliard_linear
    ...
    for grid_number = 1:number_of_grids
        ...
        solution_u = func_sol(nodes,T);
        solution_w = func_sol(nodes,T);
        ...
        for j = 1:number_of_timestep_sizes
            ...
            %the starting values for the bdf methods
            for i = 1:k
                time = (i-1)*tau;
                u(:,i) = func_sol(nodes,time);
                w(:,i) = func_sol(nodes,time);
            end
    ...
    %we compute the error in the full H^1-horm
    u_error_H1 = u_error_H1 + u_error_L2;  

    %the errors are plottet against the different mesh sizes
    x=mesh_sizes;
